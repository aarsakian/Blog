	...
	$.fn.miniTwitter = function( options ) {
	    ...
	    function createdAt (date) {
	    	var d = (new Date).getTime() - parse_date(date);
	      	...
	      	if (d>86400000) {
	      		d = thedate.getDate()+' '+month[thedate.getMonth()]+' '+thedate.getFullYear();
	      	} else if (d>3600000) {
		...
		function fetchTweetsData ( object ) {
		  ...
		  //  obj.image = obj.retweet ? object.retweeted_status.user.profile_image_url : object.profile_image_url || object.user.profile_image_url ;
		   ...
		   obj.header = '<div class="mt_header">'+obj.tweetTime+'</div>';
		//<a '+rel()+' '+target()+' class="mt_user" href="'+obj.userUrl+'">'+obj.realName+'</a> <span class="mt_screen_name">@'+obj.screenName+'</span> <div class="time">'+obj.tweetTime+'</div></div>';
		    obj.avatar = '<div class="tweet">';
		    //<div class="avatar"><a '+rel()+' '+target()+' class="mt_avatar" href="'+obj.userUrl+'"><img src="'+obj.image+'" alt="'+obj.realName+'\'s avatar" border="0"/></a></div>';
